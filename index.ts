...
const visit = (node: ts.Node, obj: any): void => {
    ...
    if (ts.isTypeLiteralNode(typeNode)) {
      typeNode.members.forEach((member) => {
        if (ts.isPropertySignature(member)) {
          const propName = (member.name as ts.Identifier).getText();
          ...
          if (propType && ts.isUnionTypeNode(propType)) {
            ...
            const values = propType.types
              ...
              .map((literalType) => (literalType.literal as ts.StringLiteral).text);
...
const convertToObject = (type: string): any => {
  const sourceFile = ts.createSourceFile(
    ...
    true,      // for parsing in a script mode
    ts.ScriptKind.TS
  );
